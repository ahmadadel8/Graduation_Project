...
@var_scope('block')
def block(x, filters, stride=1, scope=None, dropout_rate=0, lmbda=5e-4):
    x = sconv(x, None, 3, 1, stride=stride, scope='sconv',weights_regularizer= l2(lmbda))
    x = conv(x, filters, 1, stride=1, scope='conv', weights_regularizer=l2(lmbda))
    return x
...
def mobilenet(x, depth_multiplier, is_training, classes, stem,
              scope=None, reuse=None, dropout_rate=0, lmbda=5e-4):
    def depth(d):
        return max(int(d * depth_multiplier), 8)
    x = conv(x, depth(32), 3, stride=2, scope='conv1', weights_regularizer= l2(lmbda))

    x = block(x, depth(64), scope='conv2', dropout_rate=dropout_rate, lmbda=lmbda)
    x = block(x, depth(128), stride=2, scope='conv3', dropout_rate=dropout_rate, lmbda=lmbda)

    x = block(x, depth(128), scope='conv4', dropout_rate=dropout_rate, lmbda=lmbda)
    x = block(x, depth(256), stride=2, scope='conv5', dropout_rate=dropout_rate, lmbda=lmbda)
    x = block(x, depth(256), scope='conv6', dropout_rate=dropout_rate, lmbda=lmbda)
    
    x = block(x, depth(512), stride=2, scope='conv7', dropout_rate=dropout_rate, lmbda=lmbda)
    x = block(x, depth(512), scope='conv8', dropout_rate=dropout_rate, lmbda=lmbda)
    x = block(x, depth(512), scope='conv9', dropout_rate=dropout_rate, lmbda=lmbda)
    x = block(x, depth(512), scope='conv10', dropout_rate=dropout_rate, lmbda=lmbda)
    x = block(x, depth(512), scope='conv11', dropout_rate=dropout_rate, lmbda=lmbda)
    x = block(x, depth(512), scope='conv12', dropout_rate=dropout_rate, lmbda=lmbda)
    p = x

    x = block(x, depth(1024), stride=2, scope='conv13', dropout_rate=dropout_rate, lmbda=lmbda)

    x = block(x, depth(1024), scope='conv14', dropout_rate=dropout_rate, lmbda=lmbda)
    x.aliases = []
    x.p = p
    if stem: return x
...
def mobilenet25(x, is_training=False, classes=1000, lmbda=5e-4, dropout_rate=0,
                stem=False, scope=None, reuse=None):
    return mobilenet(x, 0.25, is_training, classes, stem, scope, reuse, lmbda=lmbda, dropout_rate=dropout_rate)


@var_scope('mobilenet50')
@set_args(__args_v1__)
def mobilenet50(x, is_training=False, classes=1000, lmbda=5e-4, dropout_rate=0,
                stem=False, scope=None, reuse=None):
...
@set_args(__args_v1__)
def mobilenet75(x, is_training=False, classes=1000, lmbda=5e-4, dropout_rate=0,
                stem=False, scope=None, reuse=None):
...
@set_args(__args_v1__)
def mobilenet100(x, is_training=False, classes=1000, lmbda=5e-4, dropout_rate=0,
                 stem=False, scope=None, reuse=None):
