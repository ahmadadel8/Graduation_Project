...
def darkconv(inputs, filters, kernel, name, lmbda=5e-4, dropout_rate=0):
    x = tf.keras.layers.Conv2D(filters, kernel, padding='same', use_bias=False, name=name, kernel_regularizer=tf.keras.regularizers.l2(lmbda),kernel_initializer=tf.keras.initializers.VarianceScaling(scale=1.53846))(inputs)
...
def model(inputs, is_training=True, lmbda=5e-4, dropout_rate=0): 
  ...
  x = darkconv(x, 1024, 3, name='genYOLOv2/conv9', lmbda=lmbda, dropout_rate=dropout_rate)
